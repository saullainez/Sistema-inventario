create procedure actualizar_mov_pro(IN presId  int, IN descrip text, IN fech date, IN cant int, IN anio int,
  ...
  begin
    ...
    if @error > 0
    then
      update movimiento_productos
      set PresentacionId       = presId,
          Descripcion          = descrip,
          Fecha                = fech,
          Cantidad             = cant,
          AnioCosecha          = anio,

DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `reporte_mejor_cliente`(in fechaInicio date, in fechaFin date)
begin 
    select a.ClienteId, e.EmpresaNombre,mc.Nombre,mc.TipoMovimiento,count(ClienteId) as cantidad_movimientos, sum(Monto) as total
